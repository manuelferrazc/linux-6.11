...
static struct task_struct *pick_next_task_fair(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
    ...
    struct task_struct *p;
    u64 min_predicted_burst = U64_MAX;
    ...
    list_for_each_entry(p, &rq->cfs.tasks, se.group_node) {
        ...
        if ((p->predicted_burst)-(p->start_time/1000000) < min_predicted_burst) {
            min_predicted_burst = p->predicted_burst-(p->start_time/1000000);
            next = p;
        }
    }
